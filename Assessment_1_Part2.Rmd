...
fastfood <- fastfood_data %>%
  mutate(average_calories = rowMeans(fastfood$restaurant,fastfood$calories))
...
Using the R package ggplot(), we have used this to display a graph that displays the (1) sugar variable against the (2) restaurant variablel.
``` {r}
...
### Use observations for Mcdonalds to plot sugar variable against protein with `geom_point()` asdasd
